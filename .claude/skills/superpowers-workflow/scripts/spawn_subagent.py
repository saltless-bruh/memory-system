...
"""
    ...
    try:
        with open(log_file, "w", encoding="utf-8") as log:
            ...
            # shell=False is required for the list argv above. With shell=True a
            # POSIX shell runs only cmd[0] and receives "--yolo" as $0, silently
            # dropping auto-approve so the subagent blocks on an interactive
            # prompt (audit finding NEW-1). `gemini` is resolved from PATH, so no
            # shell is needed on any platform.
