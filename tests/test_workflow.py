...
from scout.workflow import Answer, AnswerStatus, WikiEngine, answer_query
...
class FakeWiki:
    ...
    search_scopes: list[Scope] = field(default_factory=list)
    read_scopes: list[Scope] = field(default_factory=list)
...
_SCOPE = Scope(departments=frozenset({"redteam"}))
...
async def test_malformed_sources_entries_skipped() -> None:
    ...
    ans = await answer_query(
        wiki, FakeRagBackend(chunks=[]), "q", need_rag=True, scope=_SCOPE
    )
...
async def test_no_page_when_search_empty() -> None:
    ...
    ans = await answer_query(wiki, FakeRagBackend(), "anything", scope=_SCOPE)
...
async def test_page_suffices_never_calls_rag() -> None:
    ...
    ans = await answer_query(
        _wiki(), spy, "kerberoasting?", need_rag=False, scope=_SCOPE
    )
...
async def test_descends_to_rag_when_page_insufficient() -> None:
    ...
    ans = await answer_query(
        _wiki(), spy, "exact command?", need_rag=True, scope=_SCOPE
    )
...
async def test_need_rag_predicate_inspects_page() -> None:
    ...
    ans = await answer_query(
        ...
        spy,
        "q",
        need_rag=lambda p: len(p.body) < 100,
        scope=_SCOPE,
    )
...
async def test_no_source_when_rag_returns_other_file() -> None:
    ...
    ans = await answer_query(_wiki(), backend, "q", need_rag=True, scope=_SCOPE)
    assert ans.status is AnswerStatus.NO_SOURCE
    assert ans.context == ()
...
async def test_descend_with_no_sources_is_no_source() -> None:
    ans = await answer_query(
        _wiki(sources=[]), FakeRagBackend(), "q", need_rag=True, scope=_SCOPE
    )
    assert ans.status is AnswerStatus.NO_SOURCE
...
async def test_scope_threaded_through() -> None:
    ...
    wiki = _wiki()
    await answer_query(wiki, inner, "q", need_rag=True, scope=scope)
    assert len(wiki.search_scopes) == 1 and wiki.search_scopes[0] is scope
    assert len(wiki.read_scopes) == 1 and wiki.read_scopes[0] is scope
    assert inner.record_scope is scope


async def test_scope_dropping_adapter_is_rejected() -> None:
    class ScopeDroppingWiki:
        ...
        async def wiki_read(self, path: str) -> WikiPage:
            return _wiki().pages[path]

    adapter = cast(WikiEngine, ScopeDroppingWiki())
    with pytest.raises(TypeError, match="scope"):
        await answer_query(adapter, FakeRagBackend(), "q", scope=_SCOPE)


async def test_none_scope_is_rejected_before_wiki_access() -> None:
    wiki = _wiki()
    ...
    assert wiki.search_scopes == []
    assert wiki.read_scopes == []
...
async def test_many_agents_read_concurrently_consistent() -> None:
    ...
    answers = await asyncio.gather(
        *(
            answer_query(wiki, backend, f"q{i}", need_rag=True, scope=_SCOPE)
            for i in range(25)
        )
    )
