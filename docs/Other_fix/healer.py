...
def apply_heal_edit(
    ...
    page.path.write_text(
        pattern.sub(rf'\1"{safe_new}"', text, count=1), encoding="utf-8"
    )
    return True
...
def propose_heals(
    ...
    *,
    ...
    if original in PROTECTED_BRANCHES:
        print(
            f"On protected branch '{original}': branching off it (never committed here)."
        )
...
def _build_backend():
    """Construct the RAG backend for standalone/CI runs (matches repo convention).

    V1 default = RAG-Anything HTTP. For V2, set HEALER_BACKEND=pgvector once the
    pgvector_miner adapter is implemented and wire its DSN/gateway config here.
    """
    import os

    kind = os.environ.get("HEALER_BACKEND", "rag_anything_http")
    ...
    if kind == "pgvector":
        ...
        return PgVectorRlsBackend()  # wire DSN + gateway per your V2 config
    if kind == "fake":
